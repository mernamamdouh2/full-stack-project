...
const AdminEditProductsHook = (id) => {
        ...
        if (loading === false) {
            ...
            if (product) {
                if (product.status === 200) {
                    ...
                    setTimeout(() => {
                        let hash = window.location.href.split('editproduct');
                        window.location.replace(`${hash[0]}allproducts`);
                    }, 2000);
                } else {
    ...
    return [CatID, BrandID, onChangeDesName, onChangeQty, onChangeColor, onChangePriceAfter, onChangePriceBefore,
        ...
        priceBefore, qty, prodDescription, prodName]
}
